...
for %%a in (*.txt) do (
echo ^<?xml version="1.0" encoding="UTF-8"?^>^<kml xmlns="http://www.opengis.net/kml/2.2" xmlns:gx="http://www.google.com/kml/ext/2.2" xmlns:kml="http://www.opengis.net/kml/2.2" xmlns:atom="http://www.w3.org/2005/Atom"^>^<Document^>^<Style id="hl"^>^<IconStyle^>^<scale^>1.2^</scale^>^<Icon^>^<href^>http://maps.google.com/mapfiles/kml/shapes/placemark_circle_highlight.png^</href^>^</Icon^>^</IconStyle^>^<ListStyle^>^</ListStyle^>^</Style^>^<Style id="default"^>^<IconStyle^>^<scale^>1.2^</scale^>^<Icon^>^<href^>http://maps.google.com/mapfiles/kml/shapes/placemark_circle.png^</href^>^</Icon^>^</IconStyle^>^<ListStyle^>^</ListStyle^>^</Style^>^<StyleMap id="default0"^>^<Pair^>^<key^>normal^</key^>^<styleUrl^>#default^</styleUrl^>^</Pair^>^<Pair^>^<key^>highlight^</key^>^<styleUrl^>#hl^</styleUrl^>^</Pair^>^</StyleMap^>^<Folder^>>>%%~pa\new_%%~na.kml
...
>>%%~pa\new_%%~na.kml set /p=^</Folder^>^<Placemark^>^<LineString^>^<coordinates^><nul
for /f "usebackq tokens=1,* delims=," %%i in ("%%a") do (
