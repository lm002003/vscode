...
echo                内图片文件EXIF信息中的GPS信息到Excel文件中
...
echo 文件名	北纬	东经	海拔	文件位置>>"%wpath%\！GPS信息.xls"
...
for /f "tokens=*" %%f in ('dir "%wpath%\*.jpg" /a-d /b /s /on') do (
	...
	if "!tmp!" equ "1" (call :读取信息 "%%~nxf" "%%~dpf") else (
		...
		echo %%~nxf	没有GPS信息	没有GPS信息	没有GPS信息	%%~dpf>>"%wpath%\！GPS信息.xls"
	)
)
echo.
echo ·已成功完成！
echo.
echo ·存储于文件：%wpath%\！GPS信息.xls，请忽略错误对话框，打开后另存为xlsx文件！
echo.
echo ·按任意键打开提取的信息，否则请关闭窗口退出……
pause >nul
cmd /c ""%wpath%\！GPS信息.xls""
exit
...
echo %~1	!GPSN!	!GPSE!	H !GPSH!m	%~2>>"%wpath%\！GPS信息.xls"
goto :eof